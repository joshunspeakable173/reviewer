...
The final report is intentionally not organized agent by agent. It starts with cross-agent synthesis, uses judgment to surface roughly 3 to 8 high-confidence top-level issues when the evidence supports that range, moves lower-priority items into additional findings or domain-specific sections, and keeps all canonical/source finding IDs in a traceability appendix.
...
- In the external-sources appendix, include only sources and details that appear in source_objects, claim_evidence_links, or the original reviewer JSON. Do not invent bibliographic details, URLs, DOIs, publication years, titles, or source roles. If a detail is missing, leave it blank or write "not provided in reviewer evidence."
...
Use only source details present in source_objects, claim_evidence_links, or original reviewer JSON. Do not fill missing bibliographic details from memory.
